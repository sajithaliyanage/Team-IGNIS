...
try{
    ...
    //    add employee to employee table
        $sql = "INSERT INTO employee (comp_id,name,nic,gender,email,password,tel_no,image,job_cat_id,level_id,group_id,dept_id) VALUES
                (:compId,:empName,:empNIC,:empGender,:empEmail,:empPassword,:empTel,:empImage,:empJob,:empLevel,:empgid,:empDept)";
        $query = $pdo->prepare($sql);
        $query->execute(array('compId' => $empId, 'empName' => $empName, 'empNIC' => $empNIC, 'empGender' => $empGender, 'empEmail' => $empEmail,
            'empPassword' => $empPassword, 'empTel' => $empTelephone, 'empImage' => "null", 'empJob' => $empCategory, 'empLevel' => $empLevel,'empgid' =>0,'empDept' => $deptId));
...
//        $result0 = $query0->fetchAll();
        ...
        header("Location:../specialEmployee.php?job=done");

    } else {
        header("Location:../specialEmployee.php?error");
